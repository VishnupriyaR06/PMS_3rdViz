...
export default function CategoryPhaseDashboard() {
  ...
  const [phases, setPhases] = useState([]); // ✅ New state for phases
  ...
  /* ✅ Fetch phases for a category */
  const fetchPhases = async (categoryName) => {
    try {
      const res = await axios.get(
        `http://127.0.0.1:8000/api/phase_template_list/?category=${categoryName}`
      );
      setPhases(res.data);
    } catch (error) {
  ...
  const handleCreatePhase = async (e) => {
    ...
    try {
      await axios.post("http://127.0.0.1:8000/api/create_phases/", {
    ...
    <div className="p-10">
      ...
      {selectedCategory && (
        ...
        >
          ...
          {/* ✅ Phase List Display */}
          <div className="mt-10">
            <h3 className="text-2xl font-bold text-purple-600 mb-4">Phases</h3>
            ...
            {phases.length > 0 ? (
              ...
              <p className="text-gray-500 italic">No phases added yet.</p>
            )}
          </div>
        </motion.div>
      )}
                ...
                {categories.map((c) => (
                  <div
                    ...
                    onClick={() => {
                      ...
                      fetchPhases(c.name); // ✅ fetch phases when selected
                    }}
